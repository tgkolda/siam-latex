...
% Define styles compatible with SIAM's preferences; see amsthm documentation.
\newtheoremstyle{siamthm}{3pt}{3pt}{\itshape}{\parindent}{\scshape}{.}{.5em}{}
\newtheoremstyle{siamrmk}{3pt}{3pt}{}{\parindent}{\itshape}{.}{.5em}{}

% Fix the ams proof to be indented.
\makeatletter
...
% These need to be defined *after* loading cleveref
\theoremstyle{siamthm}
...
The SIAM style defines the following theorem-like environments, using
the \texttt{amsthm} package:
\begin{itemize}
\item \code{theorem}
\item \code{lemma}
\item \code{corollary}
...
\begin{example}[label=ex:theorem,bicolor,parbox=false]{Defining a theorem}
\begin{theorem}[Chu's theorem]\label{thm:a}
  Sample theorem included for illustration.  
  Numbers and parentheses, like \textup{\cref{eq:aa}}, should be set 
  in Roman type.  Here is a sample equation:
  \begin{equation*}
...
\begin{proof}
  Here is a proof that ends in an equation:
  \begin{equation}
    \label{eq:proof}
    x + y = z. \mbox{\qedhere}
  \end{equation}
...
We can talk about the theorem. \Cref{thm:a} references the theorem.
\end{example}
...
Other environments may be specified by
the \code{\newtheorem} command, numbering it together with the
\code{theorem}. For instance, a new \code{claim} can be defined as
shown in \cref{ex:newtheorem}.

\begin{example}[label=ex:newtheorem,bicolor,parbox=false]{Declaring a new ``theorem'' type}
This is a text.
...
Does \cref{claim:b} work?


\begin{claim}[Kolda's claim]\label{claim:c}
  Here is an example of a claim.
\end{claim}
\section{Lists}
