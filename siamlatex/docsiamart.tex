...
\cref{thm:mvt,def:sym,fig:tikz,fig:testfig}.
\end{example}
...
\code{\supp}.
%
This has the effect of rendering the item as a \texttt{mathop}.
\Cref{ex:bbm,ex:textmath,ex:smallmatrix,ex:matrices,ex:range,ex:ml,ex:aligned,ex:foo}
use many of the features of the \texttt{amsmath} package, which is
loaded by default. Several of the example are adapted 
from \cite{MiGo04}.
...
in-line matrix from the \texttt{amsmath} package.
...
Bigger matrices can be rendered using a number of environments from
the \texttt{amsmath} package, such as the two examples in
\cref{ex:matrices}.
...
As mentioned above, SIAM has defined some of its own
macros. \Cref{ex:range} shows the usage of the \code{\Range} macro. 
...
If instead of separate numbers, subnumbering is desired, use the
\code{subequations} environment from \texttt{amsmath} instead, as shown in \cref{ex:aligned}.
...
\Cref{ex:claim} shows how to create a new theorem-like environment and
also the \code{proof} environment.
